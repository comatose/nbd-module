...
static unsigned int debugflags = 0x0734;
#endif /* NDEBUG */
		     ...
		     int msg_flags)
{
  ...
  if (unlikely(!sock)) {
    printk(KERN_ERR "%s: Attempted %s on closed socket in sock_xmit\n",
	   nbd->disk->disk_name, (send ? "send" : "recv"));
    return -EINVAL;
  }
...
static int nbd_send_req(struct nbd_device *nbd, struct request *req)
{
  ...
  request.type = htonl(nbd_cmd(req) |
		       (( (req->cmd_flags & REQ_FUA) && (nbd->flags & NBD_FLAG_SEND_FUA)) ?
			NBD_CMD_FLAG_FUA : 0));
...
static struct request *nbd_read_stat(struct nbd_device *nbd)
{
  ...
  if (IS_ERR(req)) {
    ...
    printk(KERN_ERR "%s: Unexpected reply (%p)\n",
	   nbd->disk->disk_name, reply.handle);
    result = -EBADR;
    goto harderror;
  }
  ...
  if (ntohl(reply.error)) {
    printk(KERN_ERR "%s: Other side returned error (%d)\n",
	   nbd->disk->disk_name, ntohl(reply.error));
    req->errors++;
    return req;
  }
...
static int nbd_do_it(struct nbd_device *nbd)
{
  ...
  nbd->pid = current->pid;
  ret = sysfs_create_file(&disk_to_dev(nbd->disk)->kobj, &pid_attr.attr);
  if (ret) {
  ...
  sysfs_remove_file(&disk_to_dev(nbd->disk)->kobj, &pid_attr.attr);
  nbd->pid = 0;
  return 0;
}
		       ...
		       unsigned int cmd, unsigned long arg)
{
  ...
  case NBD_SET_FLAGS:
    ...
    if (nbd->disk)
      {
	if (nbd->flags & NBD_FLAG_ROTATIONAL)
	  queue_flag_clear_unlocked(QUEUE_FLAG_NONROT, nbd->disk->queue);
	else
	  queue_flag_set_unlocked(QUEUE_FLAG_NONROT, nbd->disk->queue);
...
MODULE_PARM_DESC(debugflags, "flags for controlling debug output (default: all on)");
#endif
